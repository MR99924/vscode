...
tickers = ["brpric3002",
           ...
           "zapric0381",
           "mypric0303",
           "phpric0301",  # Changed from "JoinMoreHistoryScaled(phpric0301, phpric0301_2012)",
           "thpric0035",
           "twpric0536",
           "pepric0288",
           "idpric0122"]
...
new_column_names = {"brpric3002": "c_inf_br",
           ...
           "zapric0381": "c_inf_za",
           "mypric0303": "c_inf_my",
           "phpric0301": "c_inf_ph", # Changed from "JoinMoreHistoryScaled(phpric0301, phpric0301_2012)": "c_inf_ph",
           "thpric0035": "c_inf_th",
           "twpric0536": "c_inf_tw",
           "pepric0288": "c_inf_pe",
           "idpric0122": "c_inf_id"
...
dm_rate_data_pop = dm_rate_data.diff()

print(dm_rate_data_pop.head())
...
column_names = pca_act_d.columns.tolist()
print(column_names)
print(pca_act_d.index)
print(pca_act_d.columns)
...
tickers = ["ih:mb:com:fis_cg_BRA:fis_bal_cg_BRA",
           ...
           "ih:mb:com:fis_cg_CHN:fis_bal_cg_CHN",
...
new_column_names = {"ih:mb:com:fis_cg_BRA:fis_bal_cg_BRA" : "fb_rat_br",
           ...
           "ih:mb:com:fis_cg_CHN:fis_bal_cg_CHN" : "fb_rat_cn",
...
new_column_names = {"brpric2998" : "ppi_rat_br",
           "clpric3088" : "ppi_rat_cl",
           ...
           "zapric0065" : "ppi_rat_za",
           "mypric0071" : "ppi_rat_my",
           "phpric0019" : "ppi_rat_ph",
           "thpric0036" : "ppi_rat_th",
           "twpric1050" : "ppi_rat_tw",
           "pepric2272" : "ppi_rat_pe",
           "idpric0932" : "ppi_rat_id"    
           }
...
for country in rate_decisions.columns:
    ...
    features = pd.concat([h_inf_qoq_3mma_d.filter(like=f'_{country_code}'),
                          c_inf_qoq_3mma_d.filter(like=f'_{country_code}'),
                          reer_data_qoq_3mma_d.filter(like=f'_{country_code}'),
                          pca_act_d.filter(like=f'_{country_code}'),
                          u_rat_12mma_sad_d.filter(like=f'_{country_code}'),
                          ca_rat_12mma_sad_d.filter(like=f'_{country_code}'),
                          fb_rat_12mma_sad_d.filter(like=f'_{country_code}'),
                          embig_data.filter(like=f'_{country_code}'),
                          curr_vol.filter(like=f'_{country_code}'),
                          res_rat_12mma_sad_d.filter(like=f'_{country_code}'),
                          ppi_rat_qoq_3mma_d.filter(like=f'_{country_code}')
                          ], axis=1)
    ...
    # Merge the target DataFrame with the results DataFrame to include dates and policy rate changes
    # results_df = results_dict[country]
    # target = target.merge(results_df, left_index=True, right_index=True, how='left')
    ...
    # **Define the bins and convert continuous labels to categorical labels** - will work on turning this back at a later stage
    bins = [-2.5, -2.25, -2, -1.75, -1.5, -1.25, -1, -0.75, -0.5, -0.25, -0.1, 0, 0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5]
    print(range(len(bins)-1))
    labels = [int(b) for b in bins[:-1]]  # Use integer labels for the bins
    try:
...
#     common_indices_df = pd.DataFrame(common_indices, columns=['common_indices'])
#     common_indices_df.to_csv(f'{country}_common_indices.csv', index = False)
