...
class Macrobond(object):
    ...
    def FetchSeries(self, ticker_list: [str]) -> pd.DataFrame:
        ...
        try:
            ...
            # Check for errors in the series
            for i, ticker in enumerate(ticker_list):
                if hasattr(series[i], 'IsError') and series[i].IsError:
                    ...
                    ticker_list = [t for j, t in enumerate(ticker_list) if j != i]
            
            if not ticker_list:  # If no valid tickers remain
                print("No valid tickers found")
                return pd.DataFrame()
            
            # Convert it to a pd.DataFrame
            df = self.__m_series_tuple_to_df(ticker_list=ticker_list, series=series)
            return df
            
        except Exception as e:
            print(f"Error in FetchSeries: {e}")
            return pd.DataFrame()
    ...
    def UploadOneOrMoreSeries(self, df: pd.DataFrame, region_list: list, description_list: list, category_list: list, frequency):
        ...
        # Upload each series
        for i, (ticker, values) in enumerate(df.iteritems()):
    ...
    def CreateDerivedMetadata(self):
        return self.CreateDerivedMetadata

    def CreateEmptyMetadata(self):
        return self.CreateEmptyMetadata

    def CreateSearchQuery(self):
        return self.CreateSearchQuery

    def CreateSeriesObject(self):
        return self.CreateSeriesObject

    def CreateSeriesObjectWithForecastFlags(self):
        return self.CreateSeriesObjectWithForecastFlags

    def DeleteOneOrMoreSeries(self):
        return self.DeleteOneOrMoreSeries

    def FetchEntities(self):
        return self.FetchEntities

    def FetchOneEntity(self):
        return self.FetchOneEntity

    def FetchOneSeriesWithRevisions(self):
        return self.FetchOneSeriesWithRevisions

    def FetchSeriesWithRevisions(self):
        return self.FetchSeriesWithRevisions

    def GetIDsOfNames(self):
        return self.GetIDsOfNames

    def GetMetadataInformation(self):
        return self.GetMetadataInformation

    def GetTypeInfo(self):
        return self.GetTypeInfo

    def GetTypeInfoCount(self):
        return self.GetTypeInfoCount
    ...
    def __m_series_tuple_to_df(self, ticker_list: [str], series) -> pd.DataFrame:
        ...
        """
        unpacked_series = [self.f_unpack_series(series[i]) for i, ticker in enumerate(ticker_list)]
