...
def analyze_data_overlap_issues(country_list, country_code_mapping, yield_list, yield_names, pol_rat, cpi_inf, act_track,
    ...
    """
    logger.info("Starting data overlap analysis")
    ...
    for country in country_list:
        ...
        logger.info(f"Analyzing data overlap for {country} (code: {country_code})")
        
        overlap_results[country] = {}
        
        for tenor_name, tenor_data in zip(yield_names, yield_list):
            logger.debug(f"Processing {tenor_name} for {country}")
            ...
            if yield_col not in tenor_data.columns:
                logger.warning(f"No {tenor_name} data found for {country}")
            ...
            if yield_series.empty:
                logger.warning(f"{tenor_name} data exists but is empty for {country}")
            ...
            logger.debug(f"Target yield data for {country} {tenor_name}: {yield_start} to {yield_end} ({yield_count} points)")
            ...
            if 'historical_forecasts' in required_features and 'historical_forecasts' in feature_sources:
                ...
                if historical_df is not None:
                    ...
                    if forecast_cols:
                        logger.debug(f"Found {len(forecast_cols)} forecast columns for {country} {tenor_name}")
                        ...
                        if not forecast_data.empty:
                            ...
                            logger.debug(f"Forecasts for {country} {tenor_name}: {forecast_start} to {forecast_end} ({forecast_count} points, {nan_percentage:.2f}% NaN)")
                            ...
                            logger.warning(f"Forecast data exists but is empty for {country} - {tenor_name}")
                    else:
                        logger.warning(f"No forecast columns found for {country} - {tenor_name}")
            ...
            for source_name in [s for s in required_features if s != 'historical_forecasts']:
                ...
                if source_df is None or source_df.empty:
                    logger.warning(f"Feature source {source_name} is empty or None")
                ...
                if not country_cols:
                    logger.warning(f"No {source_name} columns found for {country}")
                    feature_stats[source_name] = {
                ...
                if source_data.empty:
                    logger.warning(f"{source_name} data exists but is empty for {country}")
                    feature_stats[source_name] = {
                ...
                logger.debug(f"{source_name} for {country}: {source_start} to {source_end} ({source_count} points, {nan_percentage:.2f}% NaN)")
            ...
            if missing_sources:
                ...
                logger.warning(f"{issue_msg} for {country} {tenor_name}")
            ...
            if clean_features.empty:
                logger.warning(f"All feature data contains NaN values, no clean rows available for {country} {tenor_name}")
                country_tenor_result['status'] = 'No Clean Feature Data'
                country_tenor_result['issue'] = "All feature data contains NaN values"
                overlap_results[country][tenor_name] = country_tenor_result
                continue
                
            logger.debug(f"Combined features for {country} {tenor_name}: {combined_features.shape[1]} columns, {combined_features.shape[0]} rows")
            logger.debug(f"Clean features (no NaN): {clean_features.shape[0]} rows ({clean_features.shape[0]/combined_features.shape[0]*100:.2f}%)")
            ...
            if overlap_count == 0:
                ...
                logger.warning(f"NO OVERLAP found between features and target for {country} {tenor_name}")
                logger.debug(f"Yield data range: {yield_start} to {yield_end}")
                logger.debug(f"Clean features range: {features_min.strftime('%Y-%m-%d')} to {features_max.strftime('%Y-%m-%d')}")
                
                if features_max < yield_min:
                    issue = "Feature data ends before yield data begins"
                    logger.info(f"DIAGNOSIS for {country} {tenor_name}: {issue}")
                elif features_min > yield_max:
                    issue = "Feature data begins after yield data ends"
                    logger.info(f"DIAGNOSIS for {country} {tenor_name}: {issue}")
                else:
                    issue = "Date ranges overlap but no common dates"
                    logger.info(f"DIAGNOSIS for {country} {tenor_name}: {issue} - This may be due to different frequencies or missing data points")
                ...
                if features_max < yield_min:
                    ...
                    logger.debug(f"Gap size for {country} {tenor_name}: {gap_days} days")
                elif features_min > yield_max:
                ...
                logger.info(f"OVERLAP found for {country} {tenor_name}: {overlap_count} points ({overlap_pct_yield:.2f}% of yield data, {overlap_pct_features:.2f}% of clean features)")
                logger.debug(f"Overlap date range: {overlap_start} to {overlap_end}")
                ...
                if overlap_count < 30:  # Arbitrary threshold for minimum data points
                    logger.warning(f"Overlap may be too small for reliable modeling for {country} {tenor_name} ({overlap_count} points)")
    ...
    logger.info("DATA OVERLAP SUMMARY")
    logger.info(f"Total country-tenor combinations: {overlap_summary['total_combinations']}")
    logger.info(f"Successful combinations (has overlap): {overlap_summary['successful']} ({overlap_summary['successful']/overlap_summary['total_combinations']*100:.2f}%)")
    logger.info(f"Failed combinations: {overlap_summary['failed']} ({overlap_summary['failed']/overlap_summary['total_combinations']*100:.2f}%)")
    
    logger.info("Status breakdown:")
    for status, count in overlap_summary['by_status'].items():
        logger.info(f"  {status}: {count} ({count/overlap_summary['total_combinations']*100:.2f}%)")
    ...
    for country, stats in overlap_summary['by_country'].items():
        ...
        logger.info(f"  {country}: {stats['successful']}/{stats['total']} successful ({pct:.2f}%)")
    
    logger.info("By tenor:")
    for tenor, stats in overlap_summary['by_tenor'].items():
        pct = stats['successful'] / stats['total'] * 100 if stats['total'] > 0 else 0
        logger.info(f"  {tenor}: {stats['successful']}/{stats['total']} successful ({pct:.2f}%)")
    ...
    if results_df:
        ...
        logger.info("Detailed results saved to 'data_overlap_analysis.csv'")
    
    logger.info("Data overlap analysis completed")
    ...
    try:
        ...
        logger.info("Visualization saved to 'data_overlap_analysis.png'")
...
def create_data_availability_summary(country_list, country_code_mapping, yield_list, yield_names, pol_rat, cpi_inf, act_track,
    ...
    """
    logger.info("Starting data availability summary")
    ...
    if all_dfs:
        ...
        if all_start_dates and all_end_dates:
            ...
            logger.info(f"Global date range: {global_start.strftime('%Y-%m-%d')} to {global_end.strftime('%Y-%m-%d')}")
    ...
    for source_name, source_df in data_sources.items():
        ...
        logger.info(f"  Date range: {stats['start_date'] or 'N/A'} to {stats['end_date'] or 'N/A'}")
        logger.info(f"  Observations: {stats['count']}")
        logger.info(f"  Missing data: {stats['nan_pct']:.2f}%")
        if source_name == 'historical_forecasts':
            logger.info(f"  Available columns: {len(stats['columns'])}")
            # Log a sample of column names to help debug
            if stats['columns']:
                logger.debug(f"  Sample columns: {stats['columns'][:5]}...")
    
    # 3. Data availability by tenor
    logger.info("Analyzing data availability by yield tenor")
    
    for tenor_name, tenor_df in zip(yield_names, yield_list):
        stats = get_date_stats(tenor_df)
        availability['by_tenor'][tenor_name] = stats
        
        logger.info(f"{tenor_name}:")
        logger.info(f"  Date range: {stats['start_date'] or 'N/A'} to {stats['end_date'] or 'N/A'}")
        logger.info(f"  Observations: {stats['count']}")
        logger.info(f"  Missing data: {stats['nan_pct']:.2f}%")
    
    # 4. Data availability by country
    logger.info("Analyzing data availability by country")
    ...
    for country in country_list:
        ...
        # Check sources
        logger.debug(f"Data sources for {country}:")
        for source_name, source_df in data_sources.items():
            ...
            if source_name == 'historical_forecasts':
                ...
                for tenor_name in yield_names:
                    ...
                    if tenor_stats['columns']:
                        logger.debug(f"      {source_name} for {tenor_name}: {tenor_stats['start_date'] or 'N/A'} to "
                ...
                if all_forecast_stats:
                    ...
                    logger.debug(f"      {source_name}: No data available")
                    availability['by_country'][country]['sources'][source_name] = {
                        'start_date': None,
                        'end_date': None,
                        'count': 0,
                ...
                logger.debug(f"      {source_name}: {date_range} ({len(stats['columns'])} columns, {stats['count']} obs, {stats['nan_pct']:.2f}% NaN)")
        
        # Check tenors
        logger.debug(f"    Yield tenors:")
        for tenor_name, tenor_df in zip(yield_names, yield_list):
            yield_col = f"{tenor_name}_{country_code}"
            ...
            date_range = f"{stats['start_date'] or 'N/A'} to {stats['end_date'] or 'N/A'}"
            logger.debug(f"      {tenor_name}: {date_range} ({stats['count']} obs)")
    ...
    logger.info(f"  Total country-tenor combinations: {total_combinations}")
    logger.info(f"  Complete combinations (yield + all features): {complete_combinations} ({availability['global_stats']['complete_percentage']:.2f}%)")
    ...
    try:
        ...
        if data:
            ...
            logger.info("Data availability heatmap saved to 'data_availability_heatmap.png'")
            
            # Save to CSV
            availability_df.to_csv('data_availability_summary.csv')
            logger.info("Data availability summary saved to 'data_availability_summary.csv'")
        ...
        if country_completeness:
            ...
            logger.info("Data completeness chart saved to 'data_completeness_by_country.png'")
            ...
            if 'historical_forecasts' in data_sources:
                ...
                if forecast_availability:
                    ...
                    logger.info("Forecast availability heatmap saved to 'forecast_availability_heatmap.png'")
    ...
    logger.info("Data availability summary completed")
