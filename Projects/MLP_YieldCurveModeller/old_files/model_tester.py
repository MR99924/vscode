...
def prepare_feature_impacts(consolidated_results: Dict[str, Any]) -> List[Dict]:
    ...
    Parameters:
        consolidated_results (dict): Consolidated sensitivity analysis results
    
    Returns:
        list: List of dictionaries containing feature impact data
    """
    all_feature_impacts = []
...
def create_economic_theory_conformity_charts(
    ...
    plt.pie(
        ...
        autopct='%1.1f%%',
        colors=[colors[conf] for conf in conformity_counts.index]
    )
    ...
    conformity_by_feature = impacts_df.groupby('feature')['conformity'].agg(
        lambda x: (
            (x == 'Positive Match').mean() * 100, 
            (x == 'Negative Match').mean() * 100,
            (x == 'Neutral').mean() * 100,
            (x == 'Mismatch').mean() * 100
        )
    ).reset_index()
    
    conformity_by_feature.columns = ['feature', 'Positive Match', 'Negative Match', 'Neutral', 'Mismatch']
...
def determine_expected_direction(feature: str) -> int:
    ...
    Returns:
        int: Expected direction (1: positive, -1: negative, 0: neutral)
    """
...
def create_enhanced_visualizations(sensitivity_results, country, tenor, output_file_prefix, shock_results=None):
    """
    ...
    Parameters:
        ...
        tenor (str): Yield tenor
        output_file_prefix (str): Prefix for output files
        shock_results (dict, optional): Results from multiple shock levels
    """

    
    # Extract feature impacts
    feature_impacts = sensitivity_results['feature_impacts']
    expected_impacts = sensitivity_results['expected_impacts']
...
def generate_synthetic_features(feature_names: List[str], n_samples: int = 100) -> pd.DataFrame:
    ...
    Returns:
        pandas.DataFrame: Synthetic feature dataset
    """
    synthetic_data = {}
...
def extract_feature_names_from_model(model_package):
    """
...
def create_expected_impact_charts(
    all_feature_impacts: List[Dict], 
    output_dir: str
) -> pd.DataFrame:
    """
    ...
    Parameters:
        all_feature_impacts (list): List of dictionaries containing feature impact data
        output_dir (str): Directory to save visualization outputs
    
    Returns:
        pandas.DataFrame: Processed feature impacts with conformity information
    """
    ...
    def determine_expected_direction(feature: str) -> int:
        ...
        # If no match found, force a direction
        # Option 1: Raise an error to highlight unclassified features
        raise ValueError(f"Could not determine economic direction for feature: {feature}")

        # Alternative Option 2: If you prefer logging instead of raising an error
        # import logging
        # logging.warning(f"Unclassified feature forced to positive: {feature}")
        # return 1

    
    # Add expected direction to DataFrame
    impacts_df['expected_direction'] = impacts_df['feature'].apply(determine_expected_direction)
...
def modify_prepare_data(original_func):
    ...
    Parameters:
        original_func (callable): Original prepare_data function
    
    Returns:
        callable: Modified prepare_data function with diagnostic logging
    """
...
def diagnostic_model_saving(save_func):
    ...
    Parameters:
        save_func (callable): Original model saving function
    
    Returns:
        callable: Modified model saving function with diagnostic logging
    """
...
def configure_diagnostic_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler('feature_extraction_diagnostic.log')  # Log file
        ]
    )
...
def extract_feature_names(model_result, country, tenor):
    """
    Comprehensive feature name extraction with extensive logging.
    
    Parameters:
        model_result (dict): Model result dictionary
        country (str): Country name
        tenor (str): Tenor type
...
def perform_feature_sensitivity_analysis(
    model_package: Dict[str, Any], 
    feature_names: List[str],
    shock_amounts: List[float] = [25, 50, 100]
) -> Dict[str, Any]:
    """
